...
> = {
  LOW: { label: "General Notice", colors: "bg-slate-100 text-slate-600 border-slate-200", icon: Bell },
  NORMAL: { label: "Notice", colors: "bg-blue-50 text-blue-700 border-blue-200", icon: Bell },
  HIGH: { label: "Important", colors: "bg-amber-50 text-amber-700 border-amber-200", icon: AlertTriangle },
...
export default async function StudentAnnouncementsPage() {
  ...
  const announcements = await AnnouncementService.getAnnouncementsForRole("STUDENT");
    ...
    <div className="p-6 lg:p-8 space-y-6 max-w-4xl mx-auto">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          Stay informed about examination schedules, holiday notices, fee deadlines, and college events.
        </p>
      </div>
      ...
      <div className="space-y-4">
        {announcements.length === 0 ? (
          <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center text-slate-400">
            ...
            <p className="text-sm">No new notices posted at this time.</p>
          </div>
        ) : (

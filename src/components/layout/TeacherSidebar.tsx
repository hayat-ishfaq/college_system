...
const navItems = [
  ...
  { href: "/teacher/marks", label: "Marks", icon: PenTool },
...
export function TeacherSidebar() {
  ...
  return (
    <aside className="flex flex-col h-full w-60 bg-slate-900 text-white shrink-0">
      {/* Logo */}
      <div className="flex items-center gap-3 px-4 py-5 border-b border-slate-700/50">
        <div className="flex items-center justify-center w-8 h-8 bg-emerald-500 rounded-lg shrink-0">
          <GraduationCap className="w-5 h-5 text-white" />
        </div>
        <div>
          <p className="font-bold text-white text-sm leading-tight">EduManage</p>
          <p className="text-slate-400 text-[10px] leading-tight">Teacher Portal</p>
        </div>
      </div>
